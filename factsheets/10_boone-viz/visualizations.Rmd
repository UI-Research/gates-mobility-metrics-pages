...
params:
  state_county: !r c("29019", "20045", "19103", "47093")
  fake_labels: FALSE
  state_title: FALSE
---
...
palette_urbn <- {
  ...
  palette <- list(
    categorical = list(
      c("#FF934F"),
      c("#FF934F", "#CC2D35"),
      c("#FF934F", "#CC2D35", "#058ED9"),
      c("#FF934F", "#CC2D35", "#058ED9", "#848FA2", "#2D3142"),
      c("#FF934F", "#CC2D35", "#058ED9", "#848FA2", "#2D3142", "#848FA2"),
      c("#FF934F", "#CC2D35", "#058ED9", "#848FA2", "#2D3142", "#848FA2", "#848FA2"),
      c("#FF934F", "#CC2D35", "#058ED9", "#848FA2", "#2D3142", "#848FA2", "#848FA2", "#848FA2")
    ),
...
ggplot2::update_geom_defaults("bar", list(fill = "#058ED9"))
ggplot2::update_geom_defaults("col", list(fill = "#058ED9"))
```
...
plot <- data$all %>%
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.2)),
                     ...
                     limits = c(0, NA)) +
  scale_fill_manual(values = c("#FF934F", "#CC2D35", "#058ED9", "#848FA2")) +
  labs(
    x = NULL, 
    y = NULL, 
...
urbn_plot(
  ...
  urbn_note(paste(pull(info, notes2), "The Confidence Interval for this metric is not available at this time."),
    plural = TRUE),
  heights = c(25, 1, 3)
)
...
info <- m_info %>%
  filter(metric_name == "Financial security") %>%
  mutate(notes2 = str_replace_all(notes2, "<br><br>", ""))
...
info <- m_info %>%
  filter(metric_name == "Rent burden")
...
info <- m_info %>%
  filter(metric_name == "Housing instability and homelessness")
...
info <- m_info %>%
  filter(metric_name == "Family structure and stability")
...
urbn_plot(
  plot,
  urbn_source(pull(info, source_data)),
  urbn_note(paste(pull(info, notes2)),
    plural = TRUE),
  heights = c(25, 1, 4)
)
...
info <- m_info %>%
  filter(metric_name == "Neonatal health")
...
info <- m_info %>%
  filter(metric_name == "Political participation")
...
info <- m_info %>%
  filter(metric_name == "Economic inclusion")
...
info <- m_info %>%
  filter(metric_name == "Racial diversity")
...
info <- m_info %>%
  filter(metric_name == "Transportation access")
...
info <- m_info %>%
  filter(metric_name == "Transportation access-cost") %>%
  mutate(notes2 = str_replace_all(notes2, "<br><br>", ""))
...
info <- m_info %>%
  filter(metric_name == "Environmental quality")
...
info <- m_info %>%
  filter(metric_name == "Exposure to crime") 
...
info <- m_info %>%
  filter(metric_name == "Overly punitive policing") %>%
  mutate(notes2 = str_replace_all(notes2, "<br><br>", ""))
...
info <- m_info %>%
  filter(metric_name == "Access to preschool") %>%
  mutate(notes2 = str_replace_all(notes2, "<br><br>", ""))
...
info <- m_info %>%
  filter(metric_name == "Effective public education") %>%
  mutate(notes2 = str_replace_all(notes2, "<br><br>", ""))
...
plot <- data$race_ethnicity %>%
  ...
  geom_pointrange(position = position_dodge(width = 0.6)) +
  #facet_wrap(~state_county) +
  labs(x = NULL, y = NULL, title ="Metric: Average per grade change in English Language Arts achievement between third and\neighth grades") +  
  scale_y_continuous(limits = c(0, NA)) + 
  remove_ticks() +
  coord_flip() +
  theme(panel.grid.major.x = ggplot2::element_line(colour = "#dedddd"),
...
info <- m_info %>%
  filter(metric_name == "Student poverty concentration") %>%
  mutate(notes2 = str_replace_all(notes2, "<br><br>", ""))
...
info <- m_info %>%
  filter(metric_name == "College readiness") %>%
  mutate(notes2 = str_replace_all(notes2, "<br><br>", ""))
...
info <- m_info %>%
  filter(metric_name == "Access to jobs paying a living wage") %>%
  mutate(notes2 = str_replace_all(notes2, "<br><br>", ""))
