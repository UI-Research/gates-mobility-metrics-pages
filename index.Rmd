...
fpath <- 'data/mobility-metrics_race-ethnicity.csv'
...
create_tb_level3 <- function(metrics_info_df, dataset, varname_maps){
  ...
  dataset <- dataset %>% filter(subgroup_type != 'all')
  ...
  if (ci_vars == 1){
    ...
    subgroup_type_name <- temp %>% select(subgroup_type) %>% pull() %>% first()

    temp <- temp %>% select(-subgroup_type) %>% rename(!!subgroup_type_name := subgroup)
  ...
  temp %>% 
    pivot_longer(!c('state_county',all_of(subgroup_type_name)), names_to='metrics', values_to='value') %>% 
    pivot_wider(names_from = 'state_county', values_from = 'value') %>% 
    arrange(match(metrics, col_to)) %>% 
    #mutate(metrics = gsub('.*ci', ci_str, metrics)) %>% 
    mutate(metrics = gsub('.*_quality', 'Quality', metrics)) %>% 
