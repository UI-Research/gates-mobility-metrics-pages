...
env_quality_race <- glue('{subdir}/environmental-exposure_race-ethnicity.csv')
...
data_w_subgroups <- bind_rows(list(race_ethnicity=read_csv(fpath_race_ethnicity, col_types = my_col_type), 
                                   race_share_env=read_csv(env_quality_race, col_types = my_col_type),
...
create_tb_level3 <- function(metrics_info_df, dataset, varname_maps){
  ...
  if (str_detect(subgroup_this_var, fixed('|'))){
    ...
    for (sg_type in sg_lst) {
      create_tb_by_subgroup(sg_type)
    }
