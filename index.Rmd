...
fpath <- 'data/mobility-metrics-fake.csv'
...
data <- data %>%
  mutate(juvenile_arrest_rate = if_else(state == "12", as.numeric(NA), juvenile_arrest_rate))
...
create_tb_level2 <- function(metrics_info_df, dataset, varname_maps){
  ...
  if (ci_vars == 1){
    ...
    temp <- dataset %>% 
      select(matches(glue('{mb_vars}|{quality_var}|state_county')), -matches('.*sub.*|_lb|ub'))
    ...
    temp <- dataset %>% 
      select(matches(glue('{mb_vars}|state_county')), -matches('_lb|ub'))
...
create_tb_level3 <- function(metrics_info_df, dataset, varname_maps){
  ...
  sub_varname_re <- glue('{mb_vars}.*sub.*')
  mb_vars_lst <- colnames(dataset %>% 
                            select(setdiff(matches(sub_varname_re), 
  ...
  if (ci_vars == 1){
    ...
    temp <- dataset %>% 
      select(matches(glue('{sub_varname_re}|{quality_var}|state_county')), -matches('_lb|ub'))
    ...
    temp <- dataset %>%   
      select(matches(glue('{sub_varname_re}|state_county')))
    ...
    temp <- dataset %>% 
      select(matches(glue('{sub_varname_re}|state_county')), -matches('_lb|ub'))
...
income_varlist <- list(
  ...
  c(
    'pctl_20_sub_asian', 
    'pctl_20_sub_asian_ci', 
    'pctl_20_sub_white', 
    'pctl_20_sub_white_ci', 
    'pctl_20_sub_hispanic',
    'pctl_20_sub_hispanic_ci',
    'pctl_50_sub_white'
  ), 
  c(
    '20th Percentile', 
