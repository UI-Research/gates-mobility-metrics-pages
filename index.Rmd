...
fpath <- 'mobility-metrics/01_mobility-metrics.csv'
data <- read_csv(fpath) %>%
...
create_tb_level2 <- function(metrics_info_df, dataset, varname_maps){
  ...
  temp <- dataset %>% 
    ...
    metrics_desp <- glue('{metrics_desp}*')
...
create_tb_level2_homeless <- function(metrics_info_df, dataset, varname_maps){
  ...
  notes <- paste(notes, 
                    'The Confidence Intervals for Number Homeless and Share Homeless are not applicable.',
                     sep = '\n\n')
