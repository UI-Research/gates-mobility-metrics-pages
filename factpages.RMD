...
data_sub <- data %>% 
         #mutate_at(vars(matches(glue('{mb_vars}|_lb|_ub'))), funs(round(., income_decimal))) %>% 
         mutate_all(as.character) %>% 
...
m_info %>% 
     ...
     simplify() %>%
     first() %>% 
     create_tb(data, housing_varlist) 
...
create_tb_level2(m_info_lst, data_sub, housing_varlist)
...
m_info %>% 
     ...
     simplify() %>%
     first() %>% 
     create_tb(dataset=data, famstruc_varlist) 
...
create_tb_level2(m_info_lst, data_sub, famstruc_varlist)
