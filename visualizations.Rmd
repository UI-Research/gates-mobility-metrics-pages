...
data_not_multi_year <- bind_rows(data_w_subgroups, data_no_subgroups)
...
```{r financial-security, eval = FALSE}
debt_varlist <- list(
  c("share_debt_coll"),
  c("% with debt"), 
  c(
...
```{r financial-security-table2, eval = FALSE}
...
```{r financial-security-table3, eval = FALSE}
data_sub <- data %>%
...
```{r affordable-housing, eval = FALSE}
...
```{r affordable-housing-table2, eval = FALSE}
...
```{r affordable-housing-table3, eval = FALSE}
...
```{r rent-burden-table2, eval = FALSE}
...
```{r rent-burden-table3, eval = FALSE}
...
```{r housing-instability, eval = FALSE}
...
```{r housing-instability-table2, eval = FALSE}
...
```{r create_tb_multi_yr_homeless, eval = FALSE}
...
```{r health-acces-table2, eval = FALSE}
...
```{r neonatal-health-table2, eval = FALSE}
...
```{r neonatal-health-table3, eval = FALSE}
...
```{r political-participation, eval = FALSE}
...
```{r political-participation-table2, eval = FALSE}
...
```{r economic-inclusion, eval = FALSE}
...
```{r economic-inclusion-table2, eval = FALSE}
...
```{r economic-inclusion-table3, eval = FALSE}
...
```{r racial-diversity-table2, eval = FALSE}
...
```{r transportation-access, eval = FALSE}
trans_access <- list(
  c("transit_trips"),
  c("Transit trips"),
  c(
...
```{r transportation-access-table2, eval = FALSE}
...
```{r transportation-access-table3, eval = FALSE}
data_sub <- data %>%
  mutate(
    subgroup = recode(
      subgroup,
...
```{r transportation-cost, eval = FALSE}
...
```{r transportation-cost-table2, eval = FALSE}
...
```{r environemtnal-quality, eval = FALSE}
env_varlist <- list(
  c("environmental"),
  c("Air quality index"),
  c(
...
```{r environemtnal-quality-table2, eval = FALSE}
...
```{r environemtnal-quality-table3, eval = FALSE}
data_sub <- data %>%
  mutate(
    subgroup = recode(
      subgroup,
...
```{r student-poverty-table2, eval = FALSE}
...
```{r college-readiness, eval = FALSE}
...
```{r college-readiness-table2, eval = FALSE}
create_tb_level2(metrics_info_df = m_info_lst, 
                 dataset = data, 
                 varname_maps = college_readiness) 
...
```{r college-readiness-table3, eval = FALSE}
create_tb_level3(metrics_info_df = m_info_lst, 
                 dataset = data, 
                 varname_maps = college_readiness) 
...
```{r employment-table2, eval = FALSE}
...
```{r employment-table3, eval = FALSE}
