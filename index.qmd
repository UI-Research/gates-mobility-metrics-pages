...
source(here("R", "create_tb_level2.R"))
source(here("R", "create_tb_level3.R"))
source(here("R", "create_tb_multi_yr.R"))
...
data_race_poverty <- read_csv(here("mobility-metrics", "poverty-exposure_race.csv"), col_types = my_col_type) %>%
  left_join(
    read_csv(fpath_race_ethnicity, col_types = my_col_type) %>%
  ...
  mutate_at("poverty_exposure",             
            function (x) scales::percent(x, accuracy = 0.1))
  
  
data_income <- read_csv(here("mobility-metrics", "SEDA_income.csv"), col_type = my_col_type)



#read in metrics without subgroups
data_no_subgroups <- read_csv(glue("{subdir}/01_mobility-metrics.csv"), 
...
data_years <- data %>%
  filter(is_year == 1) %>%
  select(-is_year, -subgroup, -subgroup_id)
