...
#Adding this temporary chunk to replace chunk below as we run code with un-updated data which is now in the mobility-metrics/old subdirectory
...
data_recent <- read_csv(
  here("mobility-metrics", "old", "01_mobility-metrics.csv"), 
  col_type = my_col_type
) 

data_years <- read_csv(
  here("mobility-metrics", "old", "00_mobility-metrics_longitudinal.csv"),
  col_type = my_col_type,
  guess_max = 20000
) 

data_race_ethnicity <- read_csv(
  here("mobility-metrics", "old", "02_mobility-metrics_race-ethnicity.csv"), 
  col_types = my_col_type
)

data_race <- read_csv(
  here("mobility-metrics", "old", "03_mobility-metrics_race.csv"), 
  col_types = my_col_type
) 

data_race_share <- read_csv(
  here("mobility-metrics", "old", "04_mobility-metrics_race-share.csv"), 
  col_types = my_col_type
)

data_env_quality_poverty <- read_csv(
  here("mobility-metrics", "old", "environmental-exposure_poverty.csv"), 
  col_types = my_col_type
) %>% 
  left_join(
    read_csv(
...
data_education_income <- read_csv(
  here("mobility-metrics", "old", "SEDA_income.csv"), 
  col_types = my_col_type
) %>%  
  left_join(
    read_csv(
...
data_race_poverty <- read_csv(
  here("mobility-metrics", "old", "poverty-exposure_race.csv"), 
  col_types = my_col_type
) %>%
  left_join(
    read_csv(
...
```{r}
m_info <- read_csv(here::here("metrics_summary.csv"))
