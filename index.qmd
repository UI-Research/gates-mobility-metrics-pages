...
#NOTE: THIS SHOULD GO AWAY WITH UPDATED CRIME DATA!
...
#unclear why but quarto converting state_title param from FALSE to "no"
# so we take following conversion here:

#Including commented print statement to show what is going on:
#print(params$state_title)
...
#### Description of how these "*_varlist" lists are organized. ### 

# The *_varlist lists have four vectors in them. 

# The first vector has the name  of a vector of variables as they are defined in R (i.e. in snake_case). These vectors are used in the create_tb function to make the "Summary" tab. 

# The second of vectors is the "human-readable" analog of the variables in the first vector.The create_tb function uses the first vector for coding purposes but the second vector to name the rows in the table. 

# Like the first vector, the third is a vector of of snake_case variables, but this time used for the more sophisticated functions (create_tb_level2, create_tb_level3, create_tb_multi_year). This vector is different from the first because it includes confidence interval variables (denoted *_ci). 

#The fourth vector is the "human-readable" analog of the vectors listed # in the third vector and is used similarly to the second vector. 
...
rent_varlist <- list(
  summary_vars = c(
    "Share burdened for low-income households" = "share_burdened_80_ami", 
    "Share burdened for very low-income households" = "share_burdened_50_ami",
    "Share burdened for extremely low-income households" = "share_burdened_30_ami"
  ), 
  detail_vars = c(
    "Share burdened for low-income households" = "share_burdened_80_ami", 
    "Share burdened for low-income households_ci" = "share_burdened_80_ami_ci", 
    "Share burdened for very low-income households" = "share_burdened_50_ami",
    "Share burdened for very low-income households_ci" = "share_burdened_50_ami_ci",
    "Share burdened for extremely low-income households" = "share_burdened_30_ami",
    "Share burdened for extremely low-income households_ci" = "share_burdened_30_ami_ci"
  )
)
...
econ_inc_varlist <- list(
  ...
  detail_vars = c(
    "% in high poverty neighborhoods" = "poverty_exposure", 
    "% in high poverty neighborhoods_ci" = "poverty_exposure_ci"
  )
)
...
income_varlist <- list(
  ...
  detail_vars = c(
    "20th Percentile" = "pctl_20", 
    "20th Percentile_ci" = "pctl_20_ci", 
    "50th Percentile" = "pctl_50", 
    "50th Percentile_ci" = "pctl_50_ci", 
    "80th Percentile" = "pctl_80",
    "80th Percentile_ci" = "pctl_80_ci"
  )
)
