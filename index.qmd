...
params:
  state_county: !expr c("42095", "42017", "48453", "09003", "15005")
  fake_labels: FALSE
  state_title: FALSE
execute:
  echo: false
...
#AS: it looks like you set table parameters here but then use default
# arguments in the create_tb.R function
...
my_col_type <- cols(
  ...
  # AS: I got warnings for all the read_csv() calls that the hpsa_yn and hpsa_yn_quality
  # named parsers don't match the column names. Just wanted to flag to confirm this is OK
  hpsa_yn = col_double(),
...
data_env_quality_poverty <- read_csv(
  ...
  #AS: could you use data_race_ethnicity above since you've already read this csv in?
  left_join(
    read_csv(
      here("mobility-metrics", "02_mobility-metrics_race-ethnicity.csv"), 
      col_types = my_col_type
...
shell <- data_years %>%
  ...
  mutate(
    ...
    homeless_quality = 3,
    #AS: I noted that share homeless for 2021 was NA for all counties in index.html
...
if (!st) {
  ...
  #AS: since this comment was the same as in the if block above I found it a bit
  # confusing to totally follow the logic here (I can follow what is happening but
...
#AS: this winds up being empty if params$fake_labels is false because
...
#AS: it looks like the data in the summary table is the 2018 data, but the source description 
# corresponds to the 2021 data. I think that has to do with how data_recent is being constructed
# but wanted to flag to confirm OK
...
#AS: it looks like the data in the detail table is the 2018 data, but the source description 
# corresponds to the 2021 data. I think that has to do with how data_recent is being constructed
# but wanted to flag to confirm OK
...
```{r}
#| label: social-capital1-more-data
...
```{r}
#| label: social-capital2-more-data
...
#AS: in the section of the details page on this metric, there seems to be an errant {#sec-transportation-cost} in the write-up. Perhaps consider making a separate section?
```
...
# AS: this is very nitpicky, but in the other sections so far you defined the metrics info as 
# a separate argument rather than running get_vars_info() for each table
...
#AS: the link below seems incorrect, though I don't see a digital access section in the description
```
...
#AS: it doesn't seem like this gets used anywhere
...
data_race_share %>%
  ...
  mutate(
    # AS: the other sub-area recode also included 
    # `Mixed Race and Ethnicity` = "No Majority Race/Ethnicity Tracts"
    # confirm if that shouldn't be included here
    subgroup = recode(
      subgroup,
...
#AS: flagging that there's no "more data" or "subgroup" table and need to add link below
```
...
#AS: flagging that there's no "more data" or "subgroup" table
```
...
#AS: flagging that there's no "summary" or "more data"/"subgroup" table and need to add link below
